...
**Current task:** the App Store Connect push, and it is **blocked on four steps only Marc can take**. Everything else is done and verified: the phase 5 machine gate is GREEN on the current tree, the iOS archive builds and its contents verify, all five screenshot sets are captured from Release builds, the listing copy is written and inside every limit, the privacy policy is live, and the icon ships on all four platforms. What is outstanding is in `BLOCKERS.md`: the **app record** (`POST /v1/apps` is 403 for every key), the **App Group** `group.com.mdeller.phonefold`, the **iCloud container** `iCloud.com.mdeller.phonefold`, and the **App Privacy questionnaire**. The last three have no API resource at all; the two container-shaped ones read as "enabled" through the API and produce no entitlement, which is why the archive fails on iCloud rather than on anything in this repository. **Phase 5 itself is feature-complete** - every 5a, 5b and 5c item including `P5-02` - and Phase 4's `P4-16` icon is done. **The recurring finding of this session is one pattern, found four times:** a feature split across two places, where one half is complete and reads as authoritative and the other is never reached - the Watch host declared and never constructed, Handoff received and never advertised, the Studio menu bound to a model no window uses, and the Watch app built and never embedded. No build and no test sees any of them. The first three were found by asking **who calls this**; the fourth needed a real archive, because the answer was not in the source at all.
...
| P4-06 | Onboarding: three cards, and the permanent disclaimer in About | — | done |
| P4-07 | Sample gallery notes: what to listen for in each of the twelve | — | done |
| P4-08 | Accessibility: VoiceOver, Reduce Motion, Reduce Transparency, colour-blind-safe palette | — | done |
| P4-15 | Dynamic Type: the stage's fixed point sizes do not scale | P4-08 | done; capped at accessibility3, see METRICS |
| P4-09 | Mutation duet: two folds in one key, confidence delta driving dissonance | P3-09 | done |
| P4-17 | Wire the duet into the app: a mutation field, two-fold playback, the divergence readout | P4-09 | done |
| P4-10 | External display scene, `AVRoutePickerView`, connect and disconnect mid-fold | — | built; hardware check in BLOCKERS.md |
| P4-11 | Live Activity and Dynamic Island | — | done |
| P4-12 | Privacy manifest, no analytics | — | done |
| P4-16 | App icon | — | done 2026-08-31: trp-cage rendered by `make-icon` from the app's own stage, all four platforms, and visionOS's layered 512 stack. Marc: an icon does not need the icon skill |
...
| P5-02 | Higher residue cap. **Investigated 2026-08-31: PLAN's premise is void, restated below** | P5-01 | done: 128 and 256 exported and checked against eager, `Genie2Sampler` length-aware, Studio ships three buckets and the phone one |
...
| P5-06 | Drag and drop PDB and mmCIF: superpose against the prediction, per-residue RMSD | P5-01 | done |
| P5-07 | Handoff: start a fold on the phone, continue on the Mac | P5-01 | built; two devices needed to confirm (BLOCKERS.md) |

**P5-02, restated.** PLAN 5a says "higher residue cap (~640) using the unpalettised fp16 model
variant from Phase 0". That variant does not exist and was never meant to: it belongs to Phase 0
items 2 and 7, which are the **ESM-2 / ESMFold** Core ML exports, and P0-08 and P0-09 are marked
**dropped** in this file. ESMFold is not the engine any more. The 640-residue bucket was ESMFold's
length bucket, not a property of what shipped.

The real cap is Genie 2's, and it is 64: `Genie2Sampler.residues` is a static constant matching
the single exported `Genie2Step_L64.mlpackage`. P0-17 records this as "done (length 64; more
buckets outstanding)". So the task is **export more Genie 2 length buckets and make the sampler
length-aware**, with Studio offering the larger ones and the phone keeping 64, which is what
PLAN's "Studio raises the cap; the phone does not" actually means now.

Everything needed is on disk: `Tools/.cache/genie2-src` with `results/base/checkpoints/epoch=40.ckpt`,
and `Tools/export_genie2_coreml.py --lengths` already takes a list. The export is the heavy half
and is deferred while another project's job has the machine; the Swift half does not need it.
