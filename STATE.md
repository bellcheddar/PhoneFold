...
**Current task:** none outstanding that does not need Marc. **PhoneFold is on App Store Connect** (`PhoneFold: Protein Folding`, 6807353058) with both builds uploaded, VALID and attached - iOS 35.4 MB covering iPhone, iPad, the Watch app and both widget extensions, and visionOS 35.0 MB - alongside the full listing, twenty verified screenshots, free pricing in every territory, a 4+ age rating and the MIT licence across 175 territories. **Submitting for review is deliberately not automated** and is the only step left; `BLOCKERS.md` says what to read first and why screenshots must be right *before* submitting rather than after. Phase 5 is feature-complete with a GREEN machine gate, and Phase 4's `P4-16` icon is done. **The recurring finding of this whole session is one pattern, found four times:** a feature split across two places, where one half is complete and reads as authoritative and the other is never reached - the Watch host declared and never constructed, Handoff received and never advertised, the Studio menu bound to a model no window uses, and the Watch app built and never embedded. No build and no test sees any of them. Three were found by asking **who calls this**; the fourth needed a real archive. What remains in `BLOCKERS.md` is hardware: a DAW recording, two paired devices for the Watch and for Handoff, a FaceTime call for SharePlay, two signed-in iCloud accounts, and a headset for everything in 5c that can only be judged by wearing it.
